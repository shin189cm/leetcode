...
処理の全体（概念メモ）：
1. 始点のみ投入:
    最初に pq に入っているのは始点 (0, k) の1つだけ。
2. 到達可能かつ更新できるノードのみ連鎖的に追加:
    エッジを辿り、「現在の暫定記録より早く着ける」ことが確定した行き先だけを min_time を更新して pq に詰めていく。
3. すべて取り出し切って正常終了:
    新たな更新が発生しなくなり、溜まっていた候補（古い枝刈り対象を含む）を毎ターン1つずつ pop して消化し切ると、while pq が自然に終わる。
"""
