...
- Space: O(N) - 再帰呼び出しのコールスタックおよび探索中の一時リスト current の保持メモリ（出力用の領域を除く）。総空間は O(N・2^N)
...
memo:
- 作業スペース（subset）: 1つだけ存在する作業机。要素を置いたり（append）、片付けたり（pop）しながら木を探索する。
- 結果の保管庫（res）: 葉に到達した瞬間に、作業机の状態をコピー（subset.copy()）してアルバムに1枚ずつ貼り付けていく。
"""
...
class Solution:
  ...
  def subsets(self, nums: List[int]) -> List[List[int]]:
    ...
    def dfs(i: int):
      # 全要素の判定が終わったら現在の部分集合を結果に追加（シャローコピー。）
      # なおシャローコピーでも新しいリストのメモリ領域が作られるため、追加済みのリストに後続作業は影響しない
      if i >= len(nums):
